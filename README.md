...
We note the unity term in $V_a$. Oliveira defined the age-independent Verhulst factor to be $V = N(t)/N_{max}$. In his/her implementation of the model, individuals die if some real number $r \in (0, 1)$ is less than the $V$. In our case, they die if $r$ is greater than $V$. As such, both definitions of the Verhulst factor are equivalent given that the implementation of death corresponds with whatever definition we use. Here, we consider the following definition for the age-independent Verhulst factor from which the age-dependent one is derived.
...
Varying $V_a$ can be useful in, say, modelling *survivability* of some species where younger ones are more likely to die than the older ones due to environmental factors. An example utilizing this feature is by having $w_a$ go down with the age of the individuals to make them more resilient to external pressures.
