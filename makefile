...
COMPILE_LIB = $(foreach name, $(NAME_LIB0), $(COMPILE_COMM) \
	-c $(LIB_DIR)/$(name) -o $(OBJ_DIR)/$(basename $(name)).o;)
...
# Test libraries.
test_lib: lib
	@$(COMPILE_COMM) -c -o $(OBJ_DIR)/libtest.o $(TEST_DIR)/libtest.f90
	@$(COMPILE_COMM) $(OBJECT) $(OBJ_DIR)/libtest.o -o $(TEST_DIR)/test.out
	@$(TEST_DIR)/test.out
